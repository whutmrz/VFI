...
parser.add_argument("--dataset_root", type=str, default='E:/WHU/VFI-master/Dataset', help='path to dataset folder containing train-test-validation folders')
parser.add_argument("--checkpoint_dir", type=str, default='E:/WHU/VFI-master/SOTA-Super-Slomo/ckp', help='path to folder for saving checkpoints')
...
dataset = lpbfdataset.SelfDataset('train', args.dataset_root)
# sampler = DistributedSampler(dataset)
train_data = DataLoader(dataset, batch_size=args.train_batch_size, num_workers=1, pin_memory=True, drop_last=True)
args.step_per_epoch = train_data.__len__()
dataset_val = lpbfdataset.SelfDataset('test', args.dataset_root)
val_data = DataLoader(dataset_val, batch_size=args.validation_batch_size, pin_memory=True, num_workers=1)
...
def validate(epoch, best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch):
    ...
    with torch.no_grad():
        ...
        for i, (validationData, validationFrameIndex) in enumerate(tqdm(val_data)):
            ...
            Ft_p = padder.unpad(Ft_p)
            IFrame = padder.unpad(IFrame)
            #loss
            ...
            if i == len(val_data) - 1:
                ...
                Ft_p[-1] = Ft_p[-1] * 255.
                img = (Ft_p[-1].squeeze(0).detach().cpu().numpy().transpose(1, 2, 0)).astype(np.uint8)
    ...
    return psnr, (tloss / len(val_data)), best_psnr, best_psnr_epoch, ssim, best_ssim, best_ssim_epoch
...
if __name__ == '__main__':
    ...
    for epoch in range(dict1['epoch'] + 1, args.epochs):
        ...
        for trainIndex, (trainData, trainFrameIndex) in enumerate(tqdm(train_data)):
            ...
            # I1 = frame1.to(device)
            # IFrame = frameT.to(device)
            frame0, frame1, frameT = trainData[:, 0:3], trainData[:, 3:6], trainData[:, 6:]
            I0 = frame0.to(device) / 255.
            I1 = frame1.to(device) / 255.
            IFrame = frameT.to(device) / 255.
        ...
        psnr, vLoss, best_psnr, best_psnr_epoch, ssim, best_ssim, best_ssim_epoch = validate(epoch, best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch)

        valPSNR[epoch].append(psnr)
        valLoss[epoch].append(vLoss)

                #Tensorboard
        itr = trainIndex + epoch * (len(train_data))
        ...
        endVal = time.time()
        print(" epoch: %4d  PSNR_now: %0.4f  PSNR_best_epoch: %4d  PSNR_best: %0.4f  SSIM_now: %0.4f  SSIM_best_epoch: %4d  SSIM_best: %0.4f ValEvalTime: %0.2f" % (epoch, psnr, best_psnr_epoch, best_psnr, ssim, best_ssim_epoch, best_ssim, endVal - end))
        # print(" Loss: %0.6f  Iterations: %4d/%4d  TrainExecTime: %0.1f  ValLoss:%0.6f  ValPSNR: %0.4f  ValEvalTime: %0.2f LearningRate: %f" % (iLoss / args.progress_iter, trainIndex, len(train_data), end - start, vLoss, psnr, endVal - end, get_lr(optimizer)))
