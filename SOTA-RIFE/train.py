...
def train(model, local_rank, data_path, batch_size):
    if local_rank == 0:
    ...
    # train_data = DataLoader(dataset, batch_size=batch_size, num_workers=1, pin_memory=True, drop_last=True, sampler=sampler)
    ...
    for epoch in range(args.epoch):
        ...
        nr_eval += 1
        if nr_eval % 1 == 0:
            best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch = evaluate(model, val_data, nr_eval, local_rank, writer_val, best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch)
        model.save_model(log_path, local_rank)    
        # dist.barrier()

def evaluate(model, val_data, nr_eval, local_rank, writer_val, best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch):
    loss_l1_list = []
    loss_distill_list = []
    loss_tea_list = []
    psnr_list = []
    ...
    for i, data in enumerate(val_data):
        ...
        with torch.no_grad():
            ...
            gt = padder.unpad(gt)
            merged_img = padder.unpad(merged_img)
...
if __name__ == "__main__":    
    ...
    train(model, args.local_rank, args.data_path, args.batch_size)
