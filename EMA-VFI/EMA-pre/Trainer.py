...
class Model:
    def __init__(self, local_rank):
        backbonetype, multiscaletype = OUR_MODEL_CONFIG['MODEL_TYPE']
        backbonecfg, multiscalecfg = OUR_MODEL_CONFIG['MODEL_ARCH']
        self.net = multiscaletype(backbonetype(**backbonecfg), **multiscalecfg)
        self.name = OUR_MODEL_CONFIG['LOGNAME']
        self.device()

        # train
        self.optimG = AdamW(self.net.parameters(), lr=2e-4, weight_decay=0.01)
        self.lap = LapLoss()
        self.multiscaleloss = Multi_Scale_loss()
        # if local_rank != -1:
    ...
    @torch.no_grad()
    def our_inference(self, img0, img1, TTA = False, timestep = 0.5, fast_TTA = False):
        imgs = torch.cat((img0, img1), 1)
        '''
        Noting: return BxCxHxW
        '''
        if fast_TTA:
            imgs_ = imgs.flip(2).flip(3)
            input = torch.cat((imgs, imgs_), 0)
            _, _, _, preds, _, _ = self.net(input, timestep=timestep)
            return (preds[0] + preds[1].flip(1).flip(2)).unsqueeze(0) / 2.

        _, _, _, pred, _, _ = self.net(imgs, timestep=timestep)
        if TTA == False:
            return pred
        else:
            _, _, _, pred2, _, _ = self.net(imgs.flip(2).flip(3), timestep=timestep)
    ...
    def update(self, imgs, gt, learning_rate=0, training=True):
        ...
        #     return pred, loss_l1
        if training:
            flow, mask, merged, pred, af_loss, mf_loss = self.net(imgs)
            # loss_l1 = (self.lap(pred, gt)).mean()
            loss_l1 = l1_loss(pred, gt)
            for merge in merged:
                # loss_l1 += (self.lap(merge, gt)).mean() * 0.5
                loss_l1 += l1_loss(merge, gt)*0.5
            loss_multi = self.multiscaleloss(af_loss, mf_loss)
            loss = loss_l1 + 0.5 * loss_multi
            self.optimG.zero_grad()
            loss.backward()
            self.optimG.step()
            # print('loss_type = L1 + multi_scale')
            return pred, loss
