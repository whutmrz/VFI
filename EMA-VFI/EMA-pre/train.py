...
def train(model, local_rank, batch_size, data_path):
    ...
    for epoch in range(50):
        ...
        for i, imgs in enumerate(train_data):
            ...
            if local_rank == 0:
                print('epoch:{} {}/{} time:{:.2f}+{:.2f} loss:{:.4e}'.format(epoch+1, i, args.step_per_epoch, data_time_interval, train_time_interval, loss))
            step += 1
        nr_eval += 1
        if nr_eval % 1 == 0:
            best, best_epoch, best_ssim, best_ssim_epoch = evaluate(model, val_data, nr_eval, local_rank, best_psnr, best_psnr_epoch, best_ssim, best_ssim_epoch)
