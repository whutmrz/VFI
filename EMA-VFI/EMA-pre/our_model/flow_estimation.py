...
class Head(nn.Module):
    ...
    def forward(self, motion_feature, x, flow):  # /16 /8 /4
        ...
        if self.scale != 4:
            x = F.interpolate(x, scale_factor=float(self.scale) / 4, mode="bilinear", align_corners=False)
...
class small_Head(nn.Module):
    def __init__(self, in_planes, c, in_else=17):
...
class MultiScaleFlow(nn.Module):
    def __init__(self, backbone, **kargs):
        ...
        self.block1 = small_Head(64, kargs['hidden_dims'][0])
        self.unet = Unet(kargs['c'])
            ...
            if flow != None:
                ...
                if i != self.flow_num_stage - 1:
                    ...
                    flow_d, mask_d = self.block1(
                        torch.cat([t * mf[-1 - i][:B], (1 - timestep) * mf[-1 - i][B:], af[-1 - i][:B], af[-1 - i][B:]],
                                  1),
                        torch.cat((img0, img1, warped_img0, warped_img1, mask), 1), flow)
                    flow = flow + flow_d
                    mask = mask + mask_d
    ...
    def forward(self, x, timestep):
        ...
        flow = None
        # appearence_features & motion_features
        af, mf, af_loss, mf_loss = self.feature_bone(img0, img1)
        for i in range(self.flow_num_stage):
            t = torch.full(mf[-1 - i][:B].shape, timestep, dtype=torch.float).cuda()
            if flow != None:
                if i != self.flow_num_stage - 1:
        ...
        return flow_list, mask_list, merged, pred, af_loss, mf_loss