...
class MFVFI(nn.Module):
    def __init__(self, in_chans=3, embed_dims=[32, 64, 128, 256], num_heads=8,
                 mlp_ratios=4, qkv_bias=True, qk_scale=None, drop_rate=0.5,
                 attn_drop_rate=0.5, drop_path_rate=0.5, norm_layer=nn.LayerNorm,
                 depths=[4, 2, 1], window_size=6, **kwarg):
        super().__init__()

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        cur = 0

        self.conv1 = ConvBlock(in_chans, embed_dims[0], depths[0])
        self.downsample1 = nn.Sequential(
            nn.Conv2d(embed_dims[0], embed_dims[1], 3, 2, 1),
            nn.BatchNorm2d(embed_dims[1]),
            nn.PReLU(embed_dims[1])
        )
        self.conv2 = ConvBlock(embed_dims[1], embed_dims[1], depths[1])
        self.downsample2 = nn.Sequential(
            nn.Conv2d(embed_dims[1], embed_dims[2], 3, 2, 1),
            nn.BatchNorm2d(embed_dims[2]),
            nn.PReLU(embed_dims[2])
        )
        self.conv3 = ConvBlock(embed_dims[2], embed_dims[2], depths[2])
        ...
        self.encoder1 = nn.ModuleList([MotionFormerBlock(
            dim=embed_dims[1], motion_dim=2 * 32 // depths[0], num_heads=num_heads,
            window_size=7, shift_size=0 if (j % 2) == 0 else window_size // 2,
            mlp_ratio=mlp_ratios, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer)
            for j in range(depths[0])])
        self.encoder2 = nn.ModuleList([MotionFormerBlock(
            dim=embed_dims[2], motion_dim=4 * 32 // depths[1], num_heads=num_heads,
            window_size=7, shift_size=0 if (j % 2) == 0 else window_size // 2,
            mlp_ratio=mlp_ratios, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer)
            for j in range(depths[1])])
        self.encoder3 = nn.ModuleList([MotionFormerBlock(
            dim=embed_dims[3], motion_dim=8 * 32 // depths[2], num_heads=num_heads,
            window_size=7, shift_size=0 if (j % 2) == 0 else window_size // 2,
            mlp_ratio=mlp_ratios, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer)
    ...
    def forward(self, x1, x2):
        ...
        appearence_loss = []
        x1 = self.conv1(x)
        x2 = self.conv2(self.downsample1(x1))
        x3 = self.conv3(self.downsample2(x2))

        x1, h1, w1 = self.patch_embed1(x1)
        x2, h2, w2 = self.patch_embed2(x2)
        x3, h3, w3 = self.patch_embed3(x3)
        cor1 = self.get_cor((x1.shape[0], h1, w1), x1.device)
        cor2 = self.get_cor((x2.shape[0], h2, w2), x2.device)
        cor3 = self.get_cor((x3.shape[0], h3, w3), x3.device)
        for blk in self.encoder1:
            x1, x1_motion = blk(x1, cor1, h1, w1, B)
            motion_features1.append(x1_motion.reshape(2 * B, h1, w1, -1).permute(0, 3, 1, 2).contiguous())
        x1 = self.norm1(x1)
        x1 = x1.reshape(2 * B, h1, w1, -1).permute(0, 3, 1, 2).contiguous()
        motion_features1 = torch.cat(motion_features1, 1)

        for blk in self.encoder2:
            x2, x2_motion = blk(x2, cor2, h2, w2, B)
            motion_features2.append(x2_motion.reshape(2 * B, h2, w2, -1).permute(0, 3, 1, 2).contiguous())
        x2 = self.norm2(x2)
        x2 = x2.reshape(2 * B, h2, w2, -1).permute(0, 3, 1, 2).contiguous()
        motion_features2 = torch.cat(motion_features2, 1)

        for blk in self.encoder3:
            x3, x3_motion = blk(x3, cor3, h3, w3, B)
            motion_features3.append(x3_motion.reshape(2 * B, h3, w3, -1).permute(0, 3, 1, 2).contiguous())
        x3 = self.norm3(x3)
        x3 = x3.reshape(2 * B, h3, w3, -1).permute(0, 3, 1, 2).contiguous()
        motion_features3 = torch.cat(motion_features3, 1)

        appearence_features.append(x1)
        appearence_features.append(x2)
        appearence_features.append(x3)
        motion_features.append(motion_features1)
        motion_features.append(motion_features2)
        motion_features.append(motion_features3)
        x1 = self.down1(x1)
        x2 = self.down2(x2)
        motion_features1 = self.down1(motion_features1)
        motion_features2 = self.down2(motion_features2)
        appearence_loss.append(x1)
        appearence_loss.append(x2)
        appearence_loss.append(x3)
        motion_loss.append(motion_features1)
        motion_loss.append(motion_features2)
        motion_loss.append(motion_features3)
        return appearence_features, motion_features, appearence_loss, motion_loss
